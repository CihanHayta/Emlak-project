/**
 * Reference data: Turkey's 81 provinces (il) and their districts (ilçe),
 * used to power the İl -> İlçe location pickers in the admin panel (the
 * "Yeni İlan" location fields and a customer's "İstediği İl/İlçe").
 *
 * İl -> ilçe level only — no neighborhood/mahalle level here. İstanbul is
 * the one province with neighborhood-level detail, and that already lives
 * separately at src/data/istanbulLocations.js (kept as the source of truth
 * for İstanbul so the two datasets don't drift apart).
 *
 * Compiled from general knowledge of Turkey's provincial/district
 * structure, NOT independently web-verified against a live source (unlike
 * istanbulLocations.js). District lists are reasonably complete but worth
 * spot-checking against a current official source (e.g. İçişleri Bakanlığı
 * il/ilçe listesi) before treating this as authoritative for anything
 * beyond populating a dropdown.
 */
export const TURKEY_PROVINCES = [
  ...
  { name: "Yalova", districts: ["Altınova", "Armutlu", "Merkez", "Çınarcık", "Çiftlikköy", "Termal"] },

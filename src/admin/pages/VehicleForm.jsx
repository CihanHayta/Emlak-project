...
export default function VehicleForm() {
  ...
  // Aşama 6/7: medya artık backend'de object_key'e sahip gerçek satırlar —
  // yüklenebilmesi için ÖNCE gerçek bir araç ID'si gerekiyor
  // (R2 anahtarı `vehicles/{vehicleId}/...` şeklinde). Yeni ilan formunda
  // bu ID henüz yok — form açılır açılmaz status="unpublished" bir TASLAK
  // kayıt oluşturulur (public sitede zaten gizli), medya o taslağa
  // yüklenir. Kullanıcı formu terk ederse geride boş bir taslak kalabilir —
  // bilinçli bir kabul (admin panelinden elle silinebilir), karşılığında
  // "önce kaydet sonra fotoğraf yükle" gibi iki adımlı, kafa karıştırıcı
  // bir akış yerine tek, sürekli açık bir form sunuyor.
  const [vehicleId, setVehicleId] = useState(existingVehicle?.id ?? null);
  const [isCreatingDraft, setIsCreatingDraft] = useState(!existingVehicle);

  useEffect(() => {
    if (existingVehicle || vehicleId) return;
    let cancelled = false;
    addVehicle({
      category: "satilik",
      brand: "Belirtilmedi",
  ...
  async function handleSubmit(event) {
    ...
    const payload = {
      ...
      // image/images/videoUrl/documents/expertiseReportUrl artık BURADAN
      // gönderilmiyor — VehicleMediaSection kendi uçlarıyla (upload-intent/
      // confirm/cover/reorder/delete) doğrudan backend'e yazıyor, bu form
      // sadece araç bilgilerini (skaler alanları) kaydediyor.
    };

    setIsSubmitting(true);
    try {
      // vehicleId HER ZAMAN var (yeni ilanda taslak zaten oluşturulmuştu) —
      // bu yüzden burası artık HİÇBİR ZAMAN addVehicle çağırmıyor, sadece güncelliyor.
      await updateVehicle(vehicleId, payload);
      toast.success(isEditing ? "Araç güncellendi." : "Araç ilanı yayınlandı.");
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-3xl space-y-6 pb-10">
      ...
      {/* Medya — fotoğraf/video/ekspertiz raporu/belgeler, Aşama 6'nın R2 presigned-upload sistemine bağlı */}
      <section className="space-y-4 rounded-2xl border border-border p-5">
        <h3 className="font-semibold">Fotoğraf, Video ve Belgeler</h3>
        {isCreatingDraft ? (

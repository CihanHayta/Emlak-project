// src/admin/lib/matchCustomers.test.js
//
// 2026-08-06'da elle bulunan bir kayan-nokta yuvarlama hatasını (bütçe
// toleransının tam sınırındaki bir ilan yanlışlıkla dışlanıyordu — bkz.
// `Math.round` yorumunun içindeki açıklama) kalıcı bir regresyon testine
// çeviriyor.
import { describe, it, expect } from "vitest";
import { findMatchingCustomers } from "./matchCustomers";
...
describe("findMatchingCustomers", () => {
  it("bütçe sınırının TAM ucundaki bir ilanı yanlışlıkla dışlamaz (regresyon)", () => {
    // 1.500.000 * 1.15 kayan noktada 1.724.999,9999999998 çıkıyor —
    // Math.round olmadan bu müşteri 1.725.000'lik ilanı kaçırırdı.
    const listing = { type: "Daire", price: "1.725.000 TL", district: "Kadıköy" };
    const musteri = customer({ budgetMin: 1000000, budgetMax: 1500000 });

    const matches = findMatchingCustomers(listing, [musteri]);
  ...
  it("province belirtilmemiş ilan İstanbul kabul edilir", () => {
    const listing = { type: "Daire", price: "0 TL" }; // province yok

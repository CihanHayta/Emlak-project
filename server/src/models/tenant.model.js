...
export function createDefaultTenant({ name, slug, ownerUserId, phone = null, taxNumber = null }) {
  return withCreateFields({
    ...
    // Aynı şifreleme kuralı geçerli.
    facebookPage: null,
    // NOT: eskiden burada tenant'ın KENDİ Firebase projesine bağlanma bilgisi
    // tutan bir `firebase` alanı vardı (per-tenant federe Firestore/Storage
    // mimarisi). Tek-kiracılı (her müşteri = ayrı deployment) + paylaşılan
    // gerçek Firebase Auth pivotundan sonra bu konsept tamamen öldü — `tenants`
    // tablosunda karşılığı YOK (bkz. tenant.postgres.repository.js).
